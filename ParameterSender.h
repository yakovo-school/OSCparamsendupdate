...
private:
    ...
    std::atomic<float>* sendParameter;
    std::atomic<float>* valueParameter;
};
...
juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout(int valuesCount)
{
    ...
    for (int i=0; i<valuesCount; ++i) {
        //layout.add(std::make_unique<AudioParameterFloat> (IDs::StringWithIdx(IDs::value, i),            // parameterID
        //                                                        IDs::StringWithIdx(IDs::valueName, i),// parameter name
        //                                                        0.000000f,              // minimum value
        //                                                        1.000000f,              // maximum value
        //                                                        0.000001f));
        auto normRange = NormalisableRange<float>(0.0f, 1.0f, 0.0000001f);                                                        
        layout.add(std::make_unique<AudioParameterFloat> (IDs::StringWithIdx(IDs::value, i),    // parameterID
                                                          IDs::StringWithIdx(IDs::valueName, i),// parameter name
                                                          normRange, 0.5f));
        layout.add(std::make_unique<AudioParameterBool> (IDs::StringWithIdx(IDs::send, i),      // parameterID
                                                               IDs::StringWithIdx(IDs::sendName, i),     // parameter name
                                                               false));
    }
    layout.add(std::make_unique<AudioParameterInt> (IDs::oscPort,            // parameterID
